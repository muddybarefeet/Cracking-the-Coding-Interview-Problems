...
Tree.prototype.BFSelect = function(filter){
  ...
  while (queue.length) {
    ...
    //if the node passes the filter push to results
    if (filter(current.node.value, current.depth)) {
      results.push(current.node.value);
    }
  }
  console.log(results);
...
/*var x = root1.BFSelect(function (value, depth) {
    return depth === 1;
 });*/
